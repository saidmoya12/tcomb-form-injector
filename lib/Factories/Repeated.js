...
var Repeated = function (_React$Component) {
	...
	_createClass(Repeated, [{
		...
		value: function validate() {
			...
			var valueRepeated = _state.valueRepeated;

			var result = _tcombForm2.default.validate(value, this.props.type, this.props.ctx.path);
			var isValid = result.isValid();

			if (isValid) {
				if (value !== valueRepeated) {
					result = _tcombForm2.default.validate(undefined, this.props.type, 'repeated');
					isValid = false;
				}
			}
		...
		value: function renderInputs() {
			...
			var valueRepeated = _state2.valueRepeated;
