...
var Repeated = function (_t$form$Textbox) {
	...
	Repeated.prototype.shouldComponentUpdate = function shouldComponentUpdate(nextProps, nextState) {
		var should = nextState.value !== this.state.value || nextState.repeatedValue !== this.state.repeatedValue || nextState.hasError !== this.state.hasError || nextProps.options !== this.props.options || nextProps.type !== this.props.type;
	...
	Repeated.prototype._renderInputs = function _renderInputs() {
		...
		var attrs = _extends(this.props.options.attrs || {}, {
			...
			onChange: function onChange(evt) {
				_this2.onChange(evt.target.value);
			}
		});
		...
		var repeatedAttrs = _extends(this.props.options.repeatedAttrs || {}, {
			...
			value: repeatedValue,
			onChange: function onChange(evt) {
				_this2._onChangeRep(evt.target.value);
			}
		});
...
Repeated.propTypes = {
	repeatedValue: _propTypes2.default.any
};